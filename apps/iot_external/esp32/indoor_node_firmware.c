...
// --- FALLBACK НАСТРОЙКИ ---
#define DEFAULT_DEVICE_ID       "esp32-room-01"
#define DEFAULT_WIFI_SSID       "it-academy"
#define DEFAULT_WIFI_PASS       "446RKTMbb"
#define DEFAULT_MQTT_BROKER     "mqtt://172.16.22.117:1883"
...
// --- РЕЛЕ ---
#define GPIO_HEATER      17
#define GPIO_AC          18
#define GPIO_HUMIDIFIER  19
#define GPIO_WINDOW1     21
#define GPIO_WINDOW2     25
#define GPIO_EXHAUST     26

// --- MQ135 ---
#define MQ135_ADC_CHAN   ADC_CHANNEL_6
#define MQ135_HEATER_PIN 15
#define MQ135_R0         33.0f
...
static void init_relays(void) {
    ...
    gpio_set_level(GPIO_HUMIDIFIER, 0);
    gpio_set_level(GPIO_WINDOW1, 0);
    gpio_set_level(GPIO_WINDOW2, 0);
    gpio_set_level(GPIO_EXHAUST, 0);
}

static bool topic_matches(const char *actual_topic, int actual_len, const char *expected_topic) {
    size_t expected_len = strlen(expected_topic);
    return expected_len == (size_t)actual_len && strncmp(actual_topic, expected_topic, expected_len) == 0;
}

static void apply_action_mask(int action) {
    ESP_LOGI(TAG, "Action received: %d", action);
    ESP_LOGI(TAG, "  Heater:      %s", (action & (1 << 5)) ? "ON" : "OFF");
    ESP_LOGI(TAG, "  Conditioner: %s", (action & (1 << 4)) ? "ON" : "OFF");
    ESP_LOGI(TAG, "  Humidifier:  %s", (action & (1 << 3)) ? "ON" : "OFF");
    ESP_LOGI(TAG, "  Window 1:    %s", (action & (1 << 2)) ? "OPEN" : "CLOSED");
    ESP_LOGI(TAG, "  Window 2:    %s", (action & (1 << 1)) ? "OPEN" : "CLOSED");
    ESP_LOGI(TAG, "  Exhaust:     %s", (action & (1 << 0)) ? "ON" : "OFF");

    gpio_set_level(GPIO_HEATER,     (action & (1 << 5)) ? 1 : 0);
    gpio_set_level(GPIO_AC,         (action & (1 << 4)) ? 1 : 0);
    gpio_set_level(GPIO_HUMIDIFIER, (action & (1 << 3)) ? 1 : 0);
    gpio_set_level(GPIO_WINDOW1,    (action & (1 << 2)) ? 1 : 0);
    gpio_set_level(GPIO_WINDOW2,    (action & (1 << 1)) ? 1 : 0);
    gpio_set_level(GPIO_EXHAUST,    (action & (1 << 0)) ? 1 : 0);
}
...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    ...
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            esp_mqtt_client_subscribe(event->client, g_config.command_topic, 0);
            ESP_LOGI(TAG, "Subscribed to %s", g_config.command_topic);
            break;
        ...
        case MQTT_EVENT_DATA:
            if (topic_matches(event->topic, event->topic_len, g_config.command_topic)) {
                cJSON *root = cJSON_ParseWithLength(event->data, event->data_len);
                if (root == NULL) {
                    ESP_LOGW(TAG, "Invalid action JSON");
                    break;
                }

                cJSON *action_item = cJSON_GetObjectItemCaseSensitive(root, "action");
                if (cJSON_IsNumber(action_item)) {
                    apply_action_mask(action_item->valueint);
                } else {
                    ESP_LOGW(TAG, "Action payload does not contain numeric field 'action'");
                }
...
static void bme280_init_sensor(void) {
    ...
    esp_err_t res = bmp280_init(&sensor_dev, &params);
    if (res != ESP_OK) {
        ESP_LOGW(TAG, "BME280 init failed at 0x77. Trying 0x76...");
        ESP_ERROR_CHECK(bmp280_init_desc(&sensor_dev, BMP280_I2C_ADDRESS_1, 0, SDA_GPIO, SCL_GPIO));
        res = bmp280_init(&sensor_dev, &params);
    }
    ...
    if (res == ESP_OK) {
        ...
        ESP_LOGE(TAG, "BME280/BMP280 initialization failed");
    }
}
    ...
    while (true) {
        if (mqtt_client != NULL && mqtt_connected) {
            if (bmp280_read_float(&sensor_dev, &temperature, &pressure, &humidity) == ESP_OK) {
                float in_co2 = read_mq135_co2_ppm();

                cJSON *root = cJSON_CreateObject();
                if (root != NULL) {
                    cJSON_AddNumberToObject(root, "in_temp", temperature);
                    cJSON_AddNumberToObject(root, "in_hum", humidity);
                    cJSON_AddNumberToObject(root, "in_co2", in_co2);
                ...
                ESP_LOGE(TAG, "Failed to read BME280/BMP280");
            }
        }
